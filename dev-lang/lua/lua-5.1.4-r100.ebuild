...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-${MY_PV}-make-r2.patch
	epatch "${FILESDIR}"/${PN}-${MY_PV}-module_paths.patch
	...
	if ! use readline ; then
		epatch "${FILESDIR}"/${PN}-${MY_PV}-readline.patch
	fi
	...
	# are installed)
	if use static ; then
		epatch "${FILESDIR}"/${PN}-${MY_PV}-make_static-r1.patch
	fi
...
src_install() {
	emake INSTALL_TOP="${D}usr/" \
		INSTALL_LIB="${D}usr/$(get_libdir)/" \
		V="${PV}" \
		TO_BIN="lua${MY_PV} luac" \
		LIB_NAME="liblua${MY_PV}.la" \
		gentoo_install || die "emake gentoo_install failed"

	dodoc HISTORY README
	dohtml doc/*.html doc/*.png doc/*.css doc/*.gif

	insinto /usr/share/pixmaps
	newins etc/lua.ico "lua${MY_PV}.ico"
	insinto /usr/$(get_libdir)/pkgconfig
	newins etc/lua.pc "lua${MY_PV}.pc"

	newman doc/lua.1 "lua${MY_PV}.1"
	doman doc/luac.1
}
